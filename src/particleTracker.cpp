...
void ParticleTracker::remove_one_track() {
    ...
    int track_to_remove = std::min_element(sparsity_scores.begin(), sparsity_scores.end()) -
                          sparsity_scores.begin();
    ...
    std::pair<std::vector<pybind11::dict>, std::vector<Point>> ParticleTracker::merge_perceptions(
        ...
        for (const auto& perception : robot_perceptions) {
            ...
            auto perceived_humans =
                perception["perceived_humans"].cast<std::vector<pybind11::dict>>();
            ...
            for (const auto& perceived_human : perceived_humans) {
                ...
                double perceived_pos_variance = std::pow(
                    XY_STDDEV * Agent::euclidean_distance(robot_position, perceived_pos), 2);
                ...
                for (auto& merged_human : merged_perceptions.first) {
                    ...
                    if (eucl_distance < 4 * (std::sqrt(merged_pos_variance) +
                        ...
                        double updated_heading =
                            (merged_heading_variance * perceived_heading +
    ...
    Particle ParticleTracker::generate_new_particle_from_perception(
        Point perceived_pos, double position_stddev, double perceived_heading,
    ...
    std::tuple<double, double> ParticleTracker::edge_to_pose_distance_and_t(
        int edge, Point position, double heading, graph_struct& graph) {
    ...
    double ParticleTracker::calc_effective_sample_size() const {
        return 1.0 /
               std::accumulate(particle_weights.begin(), particle_weights.end(), 0.0,
